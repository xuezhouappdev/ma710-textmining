...
There are 353 records in total, based on the defined parameters. We saved all the data in the R data file ```article.df.RData``` for the future references. 
...
338 unique records are stored in our data set. 


Then we used the ```clean.documents``` function to clean the 338 records. As defined in the  ```20170321_TextMining_functions.R``` file, this function helps to change all the letters to lower case and remove the special characters, punctuation and numbers from the documents.
...
From the word cloud graph, each term's size and location differetiante the term's frequency in the collection. The larger the size of the term and the closer the term locates to the center, the higher frequency that term has. 
